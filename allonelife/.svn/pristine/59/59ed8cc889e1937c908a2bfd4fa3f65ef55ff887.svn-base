...
<span style="float:right;">
	...
	class="btn-fill-s" id="btn-modify-save-${vo.id}">수정</a>
	<a	onclick="go_delete_cancel(${vo.id})" 
	class="btn-fill-s" id="btn-delete-cancel-${vo.id}">삭제</a>
</span>
</c:if>
</div>
...
function go_delete_cancel(id){
	...
	if( $('#btn-delete-cancel-'+id).text()=='취소' ){
		...
		if( confirm('정말 삭제?') ){
			$.ajax({
				url: 'exchange/comment/delete/'+id,
				success: function(){
					comment_list();
