...
<body>
	...
	<table>
		...
		<tr>
			...
			<td colspan="9" class="left">[ ${vo.category} ${vo.purpose} ]
				${vo.title}</td>
		</tr>
		<tr>
			<th>작성자</th>
		...
		<tr>
			...
			<th>첨부파일</th>
			<td colspan="3" class="left"><label id="download">${vo.filename}${empty vo.filename ? '' 
...
<script>
   ...
   random_imglink()
	...
	<c:if test="${(login_info.userid eq vo.userid) || (login_info.admin eq 'Y')}">	
		<a class="btn-fill"
			onclick="if( confirm('정말 삭제하시겠습니까??') ) { location='delete.ex?id=${vo.id}' }">삭제</a>
	</c:if>

	<form method="post" action="list.ex">
...
function go_good(){
	...
	$('form').attr('action', 'good.ex');
	$('form').submit();
	}
}

function go_modify(){
	$('form').attr('action', 'modify.ex');
	$('form').submit();
}
